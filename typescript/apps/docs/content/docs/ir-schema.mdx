...
The predicate name is the semantic contract. Implementations can be rewritten or
optimized as long as the same inputs produce the same pass/fail result. If the
meaning changes, use a new predicate name or an explicit `args` option. There is
no hidden implementation version in the IR; behavior differences should be
...
The transform `fn` is also a semantic contract. Faster implementations are fine
when the normalized output stays the same. Output-changing behavior should use a
new transform name or an explicit option.
