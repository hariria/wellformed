...
Treat custom predicate names as stable contracts. Rewriting an implementation for speed is fine when pass/fail behavior stays the same. Behavior-changing updates should use a new predicate name or an explicit `args` option.
...
For persisted submissions, store the application schema id or schema hash that
validated the data. If you need to reproduce exact historical validation
behavior, also store the `wellformed-ts` package version or Rust crate version
used by the validating service.
...
Use the same fixtures for frontend and backend validation. A good fixture set has
valid examples, invalid examples, transformed output expectations, unknown-key
behavior, and custom predicate cases.
...
When upgrading for performance work, the expected result is no behavior change.
Run representative valid and invalid examples through both old and new runtimes.
If a predicate intentionally changes meaning, migrate schemas to a new predicate
name or explicit `args` option instead of relying on deployment timing.

Avoid runtime-only behavior in portable schemas. Custom predicates should be
pure and deterministic; permission checks, database lookups, fraud checks, and
other environment-dependent decisions belong in application code around
validation.
