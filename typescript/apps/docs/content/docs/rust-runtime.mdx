...
Register the same predicate names and semantics in every runtime that evaluates
the same schema. If a custom predicate's behavior changes, use a new name or an
explicit argument instead of silently changing what an existing name means.
For audit trails, store the schema id or hash and the crate version that
evaluated the submitted value.
