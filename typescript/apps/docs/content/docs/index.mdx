...
Validation logic, as data. Write a schema once in TypeScript. It compiles to JSON, and the same rules run in TypeScript and Rust, identically. No backend rewrite. No drift.
...
Most validation libraries hide your rules inside JavaScript functions. wellformed keeps them as JSON. That single decision is the whole product: the schema becomes portable.
