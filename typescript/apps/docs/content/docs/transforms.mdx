...
The `fn` field identifies the transform type, matching the Rust crate's
`#[serde(tag = "fn")]` format. Treat each transform name as a stable output
contract. Faster internals are fine when the normalized value stays the same;
output-changing behavior should use a new transform name or an explicit option.
